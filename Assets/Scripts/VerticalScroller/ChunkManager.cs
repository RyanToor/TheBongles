...
public class ChunkManager : MonoBehaviour
{
    ...
    private struct CalculateLRThicknessJob : IJobParallelFor
    {
        ...
        public void Execute(int index)
        {
            ...
            int lThickness = (int)math.clamp((centrePoint - chasmWidth / 2 - maxIrregularity + math.ceil((noise.cnoise(new float2(irregularitySeedL, (chunkIndex * height + (float)index / height) * irregularityFrequency)) / 2 + 0.5f) * maxIrregularity)), (width - 60) / 2, width);
            int rThickness = (int)math.clamp((width - centrePoint - chasmWidth / 2 - maxIrregularity + math.ceil((noise.cnoise(new float2(irregularitySeedR, (chunkIndex * height + (float)index / height) * irregularityFrequency)) / 2 + 0.5f) * maxIrregularity)), (width - 60) / 2, width);
