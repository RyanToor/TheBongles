...
public class Twin : MonoBehaviour
{
    ...
    [SerializeField] float collectableTrashDuration;
    [SerializeField] float collectableTrashIntensity;
    [SerializeField] AnimationCurve collectableTrashCurve;
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        if (!trashManager.loadedTrash[chunkIndex][objectIndex].isDangerous)
        {
            ...
            InputManager.Instance.Vibrate(collectableTrashIntensity, collectableTrashDuration, collectableTrashCurve);
        }
    }
}