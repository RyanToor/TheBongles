...
public class Player : MonoBehaviour
{
    ...
    private InputManager.VibrationData boostVibrate;

    // Start is called before the first frame update
    void Start()
    {
        InputManager.Instance.Jump += Boost;
    ...
    private void Boost()
    {
        if (boostLevel >= 0 && !isBoosting && boostCooldownTimer <= 0 && isloaded)
        {
            StartCoroutine(nameof(BoostActive));
        }
    }

    private IEnumerator BoostActive()
    {
        boostVibrate = InputManager.Instance.Vibrate();
        ...
        while (boostDurationTimer < boostInfo.y)
        {
            ...
            boostVibrate.Intensity = (rb.velocity.magnitude / maxSpeed) * (1 - boostDurationTimer / boostInfo.y) * boostIntensity;
    ...
    {
        InputManager.Instance.vibrations.Remove(boostVibrate);
    ...
    IEnumerator EndSequence()
    {
        ...
        InputManager.VibrationData eelVibration = InputManager.Instance.Vibrate();
        float t = 0;
        while (t < endSequenceLength)
        {
            ...
            eelVibration.Intensity = Mathf.Clamp(t / eelFadeIn, 0, 1) * eelIntensity;
            t += Time.deltaTime;
            yield return null;
        }
        InputManager.Instance.vibrations.Remove(eelVibration);
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.CompareTag("RandomTrash"))
        {
            ...
            if (trashInfo.isDangerous)
            {
                ...
                InputManager.Instance.Vibrate(dangerousTrashIntensity, dangerousTrashDuration);
                if (isBoosting)
                {
                    StopCoroutine(nameof(BoostActive));
                    StartCoroutine(BoostCooldown());
                }
            }
        }
